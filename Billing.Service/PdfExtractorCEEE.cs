...
{
    public class PdfExtractorCEEE : IPdfExtractorByDistr
    {
        public PdfExtractorCEEE()
        {
        }
        ...
        private bool FindUC(Page page, FaturaPdfData pdfData, string ancoraValor)
        {

            //encontra a âncora para a UC. Primeira seção, tem o valor da fatura abaixo da UC
            Word anchor = PdfExtractorUtils.FindWordByText(page, ancoraValor, new PdfRectangle(200, 600, page.Width / 2, page.Height));
            if (anchor != null)
            {
                Log.Debug("Anchor:" + anchor.Text);
                pdfData.UC = PdfExtractorUtils.FindOneTextByAnchor(page, anchor, -40, 60, -40, -5);
            }
            else
            {
        ...
        private void GetDadosCliente(Page page, string pageText, FaturaPdfData pdfData)
        {
            pageText = Regex.Replace(pageText.Trim(), @"\s+", " ");

            Match match = Regex.Match(pageText, @"\d{2}\/\d{4} \d{2}\/\d{2}\/\d{4} R\$(\d{0,3}[.])*\d{1,3},\d{2}");
            if (match.Success)
            {
                Log.Debug($"GetDadosCliente: {match.Value} at index {match.Index}");
                string[] infos = match.Value.Split(" ");
                
                pdfData.Valor = PdfExtractorUtils.ParseStringToDecimal(infos[2]);
                pdfData.MesRef = infos[0];
                pdfData.Vcto = PdfExtractorUtils.ParseStringToDate(infos[1]);

                FindUC(page, pdfData, infos[2]);
            }
        ...
        private void GetDadosLeitura(string pageText, FaturaPdfData pdfData)
        {
            ...
            Match match = Regex.Match(pageText, @"(\d{2}\/\d{2}\/\d{4}) (\d{2}\/\d{2}\/\d{4}) \d{1,2} (\d{2}\/\d{2}\/\d{4})");
            if (match.Success)
            {
        ...
        private void processPDF(PdfDocument document, FaturaPdfData pdfData)
        {
            ...
            string pageTextNoBreakLines = Regex.Replace(TextOut1.Trim(), @"\s+", " ");

            Log.Debug(TextOut1);

            //if (!FindUC(TextOut1, pdfData)) { }
            GetDadosCliente(page, TextOut1, pdfData);
            GetDadosLeitura(TextOut1, pdfData);

            Log.Debug("EMISSAO 0");
            Match match = Regex.Match(pageTextNoBreakLines, @"EMISSAO: \d{2}\/\d{2}\/\d{4}");
            if (match.Success)
            {
            ...
            match = Regex.Match(TextOut1, @"\d{48}|(\d{5}.(\d{5} \d{5}|\d{10}).(\d{6} \d{5}|\d{11}).(\d{6} \d{1} \d{14}|\d{21}))|(\d{12}[ ]\d{12}[ ]\d{12}[ ]\d{12})");
            if (match.Success)
            {
                Log.Debug(match.Value);
                pdfData.CodBarras = match.Value.Replace(".", "").Replace(" ", "").Replace("-", "");
            }
